...
public class BrandFilter implements IFilter<Product> {
    @Override
    public List<Product> apply(List<Product> products, String value) {
        return products.stream().filter(p -> p.getBrand().contains(value)).collect(Collectors.toList());
    }

    @Override
    public Stream<Product> applyStream(Stream<Product> products, String value) {
        return products.filter(p -> p.getBrand().contains(value));
    }
}
